		...
		Context("with boot order", func() {
			DescribeTable("[rfe_id:273][crit:high][vendor:cnv-qe@redhat.com][level:component]should be able to boot from selected disk", func(disk1, disk2 libvmi.Option, expectedConsoleText string) {
				...
				Entry("[test_id:1627]Alpine as first boot",
					libvmi.WithContainerDisk("disk2", cd.ContainerDiskFor(cd.ContainerDiskAlpine), bootOrderToDisk(1)), libvmi.WithContainerDisk("disk2", cd.ContainerDiskFor(cd.ContainerDiskCirros), bootOrderToDisk(2)),
					"Welcome to Alpine"),
				Entry("[test_id:1628]Cirros as first boot",
					libvmi.WithContainerDisk("disk2", cd.ContainerDiskFor(cd.ContainerDiskAlpine), bootOrderToDisk(2)), libvmi.WithContainerDisk("disk2", cd.ContainerDiskFor(cd.ContainerDiskCirros), bootOrderToDisk(1)),
					"cirros"),
			)
		})
	...
	Describe("[rfe_id:273][crit:high][vendor:cnv-qe@redhat.com][level:component]Delete a VirtualMachineInstance", func() {
		...
		Context("with ACPI and some grace period seconds", decorators.WgS390x, func() {
			...
			DescribeTable("[rfe_id:273][crit:medium][vendor:cnv-qe@redhat.com][level:component]should result in vmi status succeeded", func(option libvmi.Option, gracePeriod int64) {
				vmi := libvmifact.NewGuestless(option)
				...
				Consistently(matcher.ThisVMI(vmi)).WithTimeout(time.Duration(gracePeriod) * time.Second).WithPolling(time.Second).Should(
					matcher.BeInPhase(v1.Running))
